        ...
        <div className='w-12 h-12 relative' >
          {
            ...
            {
                selectedChatData.image ? (<AvatarImage src={`${HOST}/${selectedChatData.image}`} alt='profile' className='object-cover w-full h-full' />) : (<div className={`uppercase  h-12 w-12  text-lg border-[1px] flex items-center justify-center rounded-full ${getColor(selectedChatData.color)}`}>{selectedChatData.email ? selectedChatData.email.split("").shift() : selectedChatData.email.split("").shift()}</div>)
            }           
